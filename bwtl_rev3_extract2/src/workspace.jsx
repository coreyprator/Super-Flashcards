...
function WordCard({ card, role, onDrillPie, onDrillFigure, onDrillGraph, onDrillForge, onDrillRag, onNavigateWord }) {
  ...
  const figure = card.figure_link ? window.BWTL.FIGURES[card.figure_link] : null;
  const pieRoots = (card.pie_roots && card.pie_roots.length)
    ? card.pie_roots
    : (card.pie_root ? [card.pie_root] : []);
  const hasEtymology = pieRoots.length > 0;
  const showAskAI = (topic) => window.dispatchEvent(new CustomEvent('bwtl:toast', { detail: `AI researching ${topic} for "${card.word}"…` }));
  ...
  return (
    <div className="wordcard">
      <div className="wordcard-hero">
        ...
        <div className="wordcard-meta">
          <div className="wordcard-row1">
            ...
            {figure && (
              <span className="pill myth" style={{ fontSize: 10, alignSelf: 'flex-start', marginTop: 6 }}>
                <Ic.shield /> Mythological figure
              </span>
            )}
          </div>

          {/* REV-2 — IPA + anglicized as side-by-side pills */}
          <div className="pron-row" style={{ marginTop: 2 }}>
            {card.ipa && (
              <span className="pron-pill">
                <span className="lab">IPA</span>
                <span className="val">{card.ipa}</span>
                <button className="spk" title="Play IPA"><Ic.speaker /></button>
              </span>
            )}
            {card.anglicized && (
              <span className="pron-pill">
                <span className="lab">English</span>
                <span className="val angl">{card.anglicized}</span>
                <button className="spk" title="Play anglicized"><Ic.speaker /></button>
              </span>
            )}
      ...
      {/* REV-2 — Multi-root PIE display, equation style */}
      {hasEtymology && (
        <window.Etymology.MultiRootPie
          pieRoots={pieRoots}
          currentCard={card}
          onDrillPie={(r) => onDrillPie?.(r)}
          canEdit={canEdit}
        />
      )}

      {/* Empty-state for cards with no etymology (REV-2) */}
      {!hasEtymology && (
        <window.Etymology.EmptyEtymologyState card={card} onAskAI={showAskAI} />
      )}
      ...
      {/* REV-2 — Figure-only surfaces (Origin Story + Family Tree) */}
      {figure && <window.Etymology.OriginStoryPanel figure={figure} role={role} />}
      {figure && <window.Etymology.FamilyTreeGraph figure={figure} onOpenFigure={onDrillFigure} />}
      ...
      <div className="wc-section">
        ...
        {card.cognates.length === 0 ? (
          <div style={{ fontSize: 12.5, color: 'var(--fg-4)', fontStyle: 'italic' }}>
            No cognates recorded. <button className="btn xs primary" style={{ marginLeft: 6 }} onClick={() => showAskAI('cognates')}><Ic.spark /> Ask AI</button>
          </div>
        ) : (
      ...
      <div className="wc-section">
        ...
        {card.fun_facts.length === 0 ? (
          <div style={{ fontSize: 12.5, color: 'var(--fg-4)', fontStyle: 'italic' }}>
            No fun facts yet. <button className="btn xs primary" style={{ marginLeft: 6 }} onClick={() => showAskAI('fun facts')}><Ic.spark /> Ask AI</button>
          </div>
        ) : (
      ...
      {/* REV-2 — Scholarly Notes (citation list across Beekes/Watkins/Kroonen/DCC/LSJ) */}
      {hasEtymology && (
        <window.Etymology.ScholarlyNotesStack pieRoots={pieRoots} currentCard={card} />
      )}
    </div>);
