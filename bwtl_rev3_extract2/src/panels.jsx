...
function EfgPanel({ pieRootKey, currentWordId, glow, collapsed, onToggle, onClose, onPin, pinned, onOpenWord, onOpenRoot }) {
  ...
  const root = window.BWTL.PIE_ROOTS[pieRootKey];
  if (!root) {
    // No PIE root for this card (REV-2 empty-state path, e.g. fc_le_bac).
    // Render an empty-state panel rather than crashing on root.root below.
    return (
      <PanelShell variant="graph" glow={glow} collapsed={collapsed} onToggle={onToggle} onClose={onClose} onPin={onPin} pinned={pinned}
        title={<><Ic.graph /> Etymology Graph</>} meta="no root">
        <div style={{ color: 'var(--fg-3)', fontSize: 12.5, lineHeight: 1.5 }}>
          This card has no PIE root, so there's no graph subtree to show. See the empty-state in the word card for AI research options.
        </div>
      </PanelShell>
    );
  }
