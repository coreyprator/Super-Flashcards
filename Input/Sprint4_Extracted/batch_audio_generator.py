    ...
    try:
        ...
        for i, card in enumerate(cards_without_audio, 1):
            try:
                ...
                if not language:
                    print(f"[{i}/{total_cards}] ✗ {card.word} - No language found")
                    failed += 1
                    errors.append(f"{card.word}: No language found")
                    continue
                
                print(f"[{i}/{total_cards}] Processing: {card.word} ({language.name})")
                ...
                if success:
                    ...
                    failed += 1
                    error_entry = f"{card.word}: {error_msg}"
                    errors.append(error_entry)
                    print(f"  ✗ Failed: {error_msg}")
                
            except Exception as e:
                failed += 1
                error_entry = f"{card.word}: {str(e)}"
                errors.append(error_entry)
                print(f"  ✗ Error: {e}")
                db.rollback()
